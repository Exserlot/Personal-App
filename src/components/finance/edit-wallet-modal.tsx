...
export function EditWalletModal({ wallet, isOpen, onClose }: EditWalletModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in zoom-in-95 duration-200" onClick={onClose}>
      <div className="w-full max-w-sm rounded-2xl border border-border bg-background shadow-2xl ring-1 ring-border overflow-hidden relative" onClick={(e) => e.stopPropagation()}>
        ...
        {showDeleteConfirm && (
           <div className="absolute inset-0 bg-background z-10 flex flex-col items-center justify-center p-6 animate-in fade-in slide-in-from-bottom-4 duration-200">
              {/* ... (keep delete confirmation content same) ... */}
                ...
                {Object.values(THAI_BANKS).map((bank) => (
                   <button
                     ...
                     onClick={() => {
                       ...
                       setName(prev => prev === wallet.name ? bank.nameEN : prev); // Only auto-fill if name hasn't been changed significantly? Or just simplify:
                       // Actually, let's just set the icon. User might want to keep "Main Account" as name.
                       // Maybe prompt or separate action? Let's just set icon for now.
        ...
        <div className="flex items-center justify-between p-6 pb-2">
          <h3 className="text-lg font-bold">Edit Wallet</h3>
