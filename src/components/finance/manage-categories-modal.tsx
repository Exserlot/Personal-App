          ...
          <div className="absolute inset-0 bg-background/95 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center p-6 animate-in fade-in zoom-in-95 duration-200 rounded-2xl">
            ...
            <div className="flex w-full gap-3 max-w-xs">
              ...
              <DeleteButton
                ...
                loading={loading}
                disabled={loading}
                className="flex-1 bg-rose-600 text-white dark:bg-rose-600 dark:text-white"
                icon={null}
                label="Confirm"
              />
            </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-1 min-h-[200px]">
          {filteredCategories.map((c) => {
            ...
            return (
              <div
                key={c.id}
                className="flex flex-col p-3 rounded-xl border border-border bg-card/50 transition-colors"
              >
                {editingId === c.id ? (
                    ...
                    <div className="flex gap-2 overflow-x-auto py-1.5 px-1 scrollbar-hide">
                      {COLORS.map((col) => (
                        <button
                          ...
                          className={cn(
                            "h-6 w-6 rounded-full ring-2 flex-shrink-0",
                            editColor === col
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div
                        className="h-8 w-8 rounded-full shadow-sm ring-1 ring-border/50 flex items-center justify-center text-white"
          ...
          {isCreating ? (
            <div className="animate-in slide-in-from-bottom-2 fade-in space-y-3 bg-secondary/30 p-3 rounded-xl">
              <input
                ...
                className={cn(PREMIUM_INPUT_CLASS, "px-3 py-2 mb-2")}
              />
              ...
              <div className="flex gap-2 overflow-x-auto py-1.5 px-1 scrollbar-hide">
                {COLORS.map((col) => (
                  <button
                    ...
                    className={cn(
                      "h-7 w-7 rounded-full ring-2 transition-all flex-shrink-0",
                      newColor === col
                        ? "ring-primary scale-110"
                        : "ring-transparent",
                    )}
